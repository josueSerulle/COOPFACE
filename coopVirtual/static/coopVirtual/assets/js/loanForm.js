$(document).ready(function () {
    ...
    $(document).on("change", ".cedula", function () {
        if($(this).valid()) {
            $.ajax({
                method: "GET",
                url: `${getUrl()}loand_form/person/${$(this).val().replace('-','').replace('-','')}`,
                statusCode: {
                    200: (response) => {
                        if(Object.keys(response).length > 0 && typeof response !== 'undefined'){

                            let parent = $(this).closest('.row');

                            parent.find("#nombre").val(response.person[0].nombre);
                            parent.find("#apellido").val(response.person[0].apellidos);
                            parent.find("#correo").val(response.person[0].correo);
                            parent.find("#celular").val(response.person[0].celular);
                            parent.find("#telefono").val(response.person[0].telefono);
                        } else {
                            parent.find("#nombre").val('');
                            parent.find("#apellido").val('');
                            parent.find("#correo").val('');
                            parent.find("#celular").val('');
                            parent.find("#telefono").val('');
                        }
                    }
                }
            });
